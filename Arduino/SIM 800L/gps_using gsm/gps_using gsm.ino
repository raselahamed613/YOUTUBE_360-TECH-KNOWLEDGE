...
void setup() {
  ...
  responce = SIM800_send("AT+SAPBR=3,1,\"APN\",\"gpinternet\" ");  //Set VPN options => 'RCMNET' 'www'
  ...
  Serial.print("Responce:");
  Serial.println(responce);  //Open bearer Profile
  ...
  // responce = SIM800_send("AT+CIPGSMLOC=1,1");  //loction

  // Serial.print("Responce:");
  // Serial.println(responce);
  responce = SIM800_send("+CENG=2");
...
void loop() {
  ...
  if (SIM800.available()) {  //Check if the SIM800 Module is telling anything
...

    {
      ...
      responce = "";
      Latitude = "";
      Longitude = "";  //initialise all string to null
      ...
      prepare_message();
      delay(1000);  //use prepare_message funtion to prepare the link with the obtained LAT and LONG co-ordinates
      ...
      SIM800.println("AT+CMGS=\"+8801684726517\"");  //Send SMS to this number
